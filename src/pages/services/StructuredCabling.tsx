...
import { Cable, CheckCircle2, Building2, Factory, Hotel, Briefcase } from 'lucide-react';
import { Link } from 'react-router-dom';

const StructuredCabling = ({ t, lang }: { t: any, lang: string }) => {
  const details = t.serviceDetails.cabling;
  const isRtl = lang === 'ar';

  const industries = [
    { name: isRtl ? 'المكاتب الشركات' : 'Corporate Offices', icon: Briefcase },
    { name: isRtl ? 'مراكز البيانات' : 'Data Centers', icon: Building2 },
    { name: isRtl ? 'التصنيع' : 'Manufacturing', icon: Factory },
    { name: isRtl ? 'الضيافة' : 'Hospitality', icon: Hotel }
  ];
    ...
    <div className="min-h-screen bg-slate-50 pt-24 pb-20" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
          ...
          >
            <img 
              src="https://picsum.photos/seed/cabling/1000/800" 
              alt={details.title} 

...
import { Wifi, CheckCircle2, Building, ShoppingBag, GraduationCap, Plane } from 'lucide-react';
import { Link } from 'react-router-dom';

const WirelessCommunication = ({ t, lang }: { t: any, lang: string }) => {
  const details = t.serviceDetails.wireless;
  const isRtl = lang === 'ar';

  const industries = [
    { name: isRtl ? 'الضيافة' : 'Hospitality', icon: Building },
    { name: isRtl ? 'البيع بالتجزئة' : 'Retail', icon: ShoppingBag },
    { name: isRtl ? 'التعليم' : 'Education', icon: GraduationCap },
    { name: isRtl ? 'الطيران' : 'Aviation', icon: Plane }
  ];
    ...
    <div className="min-h-screen bg-slate-50 pt-24 pb-20" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
          ...
          >
            <img 
              src="https://picsum.photos/seed/wireless/1000/800" 
              alt={details.title} 

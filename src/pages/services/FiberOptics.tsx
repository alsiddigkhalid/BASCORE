...
import { Network, CheckCircle2, Globe, Zap, Shield, Activity } from 'lucide-react';
import { Link } from 'react-router-dom';

const FiberOptics = ({ t, lang }: { t: any, lang: string }) => {
  const details = t.serviceDetails.fiber;
  const isRtl = lang === 'ar';

  const industries = [
    { name: isRtl ? 'الاتصالات' : 'Telecommunications', icon: Globe },
    { name: isRtl ? 'المدن الذكية' : 'Smart Cities', icon: Zap },
    { name: isRtl ? 'المؤسسات المالية' : 'Financial Institutions', icon: Shield },
    { name: isRtl ? 'الرعاية الصحية' : 'Healthcare', icon: Activity }
  ];
    ...
    <div className="min-h-screen bg-slate-50 pt-24 pb-20" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
          ...
          >
            <img 
              src="https://picsum.photos/seed/fiber/1000/800" 
              alt={details.title} 
